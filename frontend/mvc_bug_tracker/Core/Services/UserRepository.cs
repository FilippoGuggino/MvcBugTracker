...
namespace mvc_bug_tracker.Core.Repositories
{
    ...
    {
        private readonly AWS _cloudConfig;
        ...
        public UserRepository(IOptions<AWS> appConfig, UserContextManager userManager, IHttpContextAccessor httpContextAccessor)
        {
            ...
            _provider = new AmazonCognitoIdentityProviderClient(
                _cloudConfig.AppClientId, _cloudConfig.AppClientSecret, RegionEndpoint.GetBySystemName(_cloudConfig.Region));
        ...
        private async Task<Tuple<CognitoUser, AuthenticationResultType>> AuthenticateUserAsync(string emailAddress, string password)
        {
            CognitoUser user = new CognitoUser(
                ...
                _userPool, 
                _provider,
                clientSecret: _cloudConfig.AppClientSecret);
